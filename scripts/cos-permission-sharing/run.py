...
s3_keys = {
    "user1": {"ACCESS_KEY": "", "SECRET_KEY": ""},  # publc cos key
    "user2": {"ACCESS_KEY": "", "SECRET_KEY": ""},  # personal private key
}
...
class cos_client:
    ...
    def list_objects_by_bucket(self, bucket=""):
        if bucket != "":
            return [
                x["Key"] for x in self.client.list_objects_v2(Bucket=bucket)["Contents"]
            ]
                ...
                "Statement": [
                    {
                        ...
                        "Principal": {"AWS": ["arn:aws:iam:::user/%s" % (uuid)]},
                    }
                ],
            }
    ...
    def get_obj_in_bucket(self, bucket="", okey=""):
        ...
        print(
            ">>> geting object %s: ContentLength=%s, ContentType=%s"
            % (okey, res["ContentLength"], res["ContentType"])
        )
...
print(
    "user1 check %s" % testing_bkt, s3_user1.list_objects_by_bucket(bucket=testing_bkt)
)
...
print(
    "user2 check %s" % testing_bkt, s3_user2.list_objects_by_bucket(bucket=testing_bkt)
)
