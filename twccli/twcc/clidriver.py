...
class ServiceOperation:
    ...
    def _load(self):
        ...
        self.valid_funcs = [_ava_funcs_[x]["name"] for x in range(len(_ava_funcs_))]

        self.valid_http_verb = dict(
            [
                (_ava_funcs_[x]["name"], _ava_funcs_[x]["http_verb"])
                for x in range(len(_ava_funcs_))
            ]
        )
        self.url_format = dict(
            [
                (_ava_funcs_[x]["name"], _ava_funcs_[x]["url_type"])
                for x in range(len(_ava_funcs_))
            ]
        )
        self.url_ptn = dict(
    ...
    def _to_curl(self, t_api, t_headers, t_data=None, mtype="get"):
        ...
        if isNone(t_data):
            ...
            return command.format(
                method=mtype.upper(), headers=headers, data=t_data, uri=t_api
            )
        else:
            command = (
                "curl -s -X {method} \\\n -H {headers} \\\n -d '{data}' \\\n '{uri}'"
    ...
    def _api_act(
        self, t_api, t_headers, t_params, t_data=None, mtype="get", show_curl=False
    ):
        ...
        if mtype == "get":
            r = requests.get(
                t_api, params=t_params, headers=t_headers, verify=ssl_verify_mode
            )
            ...
            r = requests.delete(
                t_api, headers=t_headers, params=t_params, verify=ssl_verify_mode
            )
        elif mtype == "patch":
            r = requests.patch(
                t_api,
    ...
    def doAPI(
        ...
        if http == "get" or http == "put" or http == "patch" or http == "delete":
            mkAPIUrl_v3 = True
        t_url = self.mkAPIUrl(
            site_sn, api_host, func, url_dict=url_dict, is_v3=mkAPIUrl_v3
        )
        ...
        res = self._api_act(
            t_url, t_header, t_params=url_ext_get, t_data=data_dict, mtype=http
        )
        import sys
    ...
    def mkAPIUrl(
        self, site_sn=None, api_host=None, func=None, url_dict=None, is_v3=True
    ):
        ...
        # need to migrate /v3/
        if (
            "PLATFORM" in url_parts
            and url_parts["PLATFORM"]
            in [
                "openstack-taichung-default-2",
                "openstack-taichung-default",
                "k8s-D-twcc",
                "k8s-taichung-default",
            ]
            and "sites" in url_parts["FUNCTION"]
        ):
            if is_v3:
