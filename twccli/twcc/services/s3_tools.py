...
class S3:
    ...
    def list_bucket(self, show_versioning=False):
        ...
        for x in response["Buckets"]:
            ...
            for y in x:
                ...
                if show_versioning:
                    if "Name" in ele:
                        ans = self.get_versioning(ele["Name"])
                        if "Status" in ans:
                            ele["Versioning"] = ans["Status"]
                        else:
                            ele["Versioning"] = "Suspended"
    ...
    def put_obj_acl(self, okey, bkt, is_public=False):
        ...
        res = self.s3_cli.put_object_acl(ACL=acl_string, Bucket=bkt, Key=okey)
        return res
                    ...
                    for key in ele:
                        if not key in not_show:
                            if key == "Size":
                                ...
                                data[key] = ele[key]
                    data["Versioning"] = len(
                        self.s3_cli.list_object_versions(
                            Bucket=bucket_name, Prefix=ele["Key"]
                        )["Versions"]
                    )
                    tmp.append(data)
        if tmp:
    ...
    def upload_bucket(
        self, file_name=None, bucket_name=None, key=None, path=None, r=False
    ):
        """Upload to S3
        ...
        if r == True:
            if os.path.isdir(path):
                ...
                for singleFilePath in out.decode("utf-8").split("\n"):
                    if (
                        os.path.isdir(singleFilePath) == False
                        and len(singleFilePath) > 0
                    ):
                        if os.path.isabs(path) == False:
                            ...
                            remotePath = os.path.dirname(path) + "/" + singleFilePath
                        ...
                        try:
                            self.s3_cli.upload_file(localPath, bucket_name, remotePath)
    ...
    def download_bucket(
        self, bucket_name=None, key=None, file_name=None, path=None, r=False
    ):
        """Download from S3
    ...
    def del_object(self, bucket_name, file_name):
        ...
        """
        res = self.s3_cli.delete_object(Bucket=bucket_name, Key=file_name)
    ...
    def _set_versioning(self, bucket_name, state="Suspended"):
        ...
        if state in def_state:
            return self.s3_cli.put_bucket_versioning(
                Bucket=bucket_name, VersioningConfiguration={"Status": state}
            )
