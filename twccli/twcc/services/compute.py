...
def chkPortPair(x):
    return (
        True
        if type(x) == type({})
        and len(set(["exposed", "inner"]).intersection(set(x.keys()))) == 2
        else False
    )
...
class GpuSite(GpuService):
    ...
    def getSolList(mtype="dict", name_only=False, reverse=False):
        ...
        ext_cntr_sol = set(
            ["Preemptive GPU", "Custom Image", "Preemptive GPU(Custom Image)"]
        )
    ...
    def getGpuDefaultHeader(flavor, sol_name, gpus="1"):
        ...
        return dict(
            [("x-extra-property-%s" % (x), gpu_default[x]) for x in gpu_default.keys()]
        )
    ...
    def mkS3MountFormat(alist):
        if len(alist) > 0:
            return json.dumps(
                [{"name": x, "mountpath": "/mnt/s3/%s" % (x)} for x in alist]
            )
        else:
            return "[]"
    ...
    def getIpBindAttr(port_mapping, pod_name="_UNDEF_"):
        default_assign_ip = {"action": "associateIP", "pod_name": pod_name, "ports": []}
    ...
    def getConnInfo(self, site_id, ssh_info=False):
        ...
        if "Service" in info_detail:
            ...
            if not ssh_info:
                ...
                info_service = [
                    x["port"]
                    for x in info_detail["Service"][0]["ports"]
                    if x["target_port"] == 22
                ][0]
    ...
    def getPodName(self, site_id):
        ...
        if (
            "Pod" in detail
            and len(detail["Pod"]) == 1
            and detail["Pod"][0]["status"] == "running"
        ):
...
class VcsSite(CpuService):
    ...
    def _do_list_solution(self, sol_id):
        ...
        return (
            getProjectSolution["site_extra_prop"]
            if "site_extra_prop" in getProjectSolution
            else []
        )
    ...
    def extend_vcs_flavor(name2id, flv_in_sol):
        ...
        alternative_names = {
            "v.super": [
                ...
                "02vCPU_016GB_MEM_LIC",
            ],
            "v.xsuper": [
                "v.xsuper",
                "04_vCPU_032GB_MEM_100GB_HDD",
                "04vCPU_032GB_MEM_LIC",
            ],
            "v.2xsuper": [
                "v.2xsuper",
                "08_vCPU_064GB_MEM_100GB_HDD",
                "08vCPU_064GB_MEM_LIC",
            ],
            "v.4xsuper": [
                "v.4xsuper",
                "16_vCPU_128GB_MEM_100GB_HDD",
                "16vCPU_128GB_MEM_LIC",
            ],
            "v.8xsuper": [
                "v.8xsuper",
                "32_vCPU_256GB_MEM_100GB_HDD",
        ...
        for ele in extra_prop:
            if ele == "flavor":
                ...
                res["x-extra-property-{}".format(ele)] = [
                    x.split(")")[1] for x in extra_prop[ele] if re.search("public", x)
                ]
...
class VcsServerNet(CpuService):
    ...
    def action(self, site_id, is_bind=True, eip_id=None):
        ...
        self.http_verb = "put"
        self.url_dic = {self._func_: server_id, "action": ""}
        self.data_dic = {"action": "associateIP" if is_bind else "disassociateIP"}
...
class VcsSecurityGroup(CpuService):
    ...
    def addSecurityGroup(self, secg_id, port_min, port_max, cidr, protocol, direction):
...
class SecurityGroups(CpuService):
    ...
    def list(self, ids=None, secg_type=None, isall=False):
        ...
        if secg_type == "detail" or (secg_type == None and not ids == ()):
            ...
            filter_type = ""
            return self.get_all_secg("project", filter_type, isall, my_username)
        else:
            self.http_verb = "get"
    ...
    def addRule(self, secg_id, port_min, port_max, cidr, protocol, direction):
    ...
    def get_all_secg(self, secg_type, filter_type, isall, my_username):
        ...
        filter_type_dict = {"loadbalancer": "LB", "server": "VM", "site": "CCS"}
        if not filter_type == "":
            all_secgs = [
                secg
                for secg in all_secgs
                if secg["type"] == filter_type_dict[filter_type]
            ]
        if isall:
            return all_secgs
        else:
            return [x for x in all_secgs if x["user"]["username"] == my_username]
...
class VcsSolutions(CpuService):
    ...
    def get_images_by_sol_name(self, sol_name):
        ...
        return sorted(
            sorted(return_ans, key=lambda d: d["Provider"], reverse=True),
            key=lambda d: d["VCSi Name"],
        )
            ...
            for y in all_flvs:
                if x == y["name"]:
                    ...
                    return_ans.append(
                        {
                            "flavor name": x,
                            "spec": get_flavor_string(
                                res_obj["gpu"], res_obj["cpu"], res_obj["memory"]
                            ),
                        }
                    )
    ...
    def get_info_by_sol_name(self, sol_name, field_name="flavor"):
        sols = self.list()
        found_sol_id = [x for x in sols if str(x["name"]).lower() == sol_name.lower()]
...
class LoadBalancers(CpuService):
    ...
    def create(
        self, vlb_name, pools, vnet_id, listeners, vlb_desc, json_data=None, eip_id=None
    ):
        self.http_verb = "post"
        self.data_dic = {
            "name": vlb_name,
...
class Volumes(CpuService):
    ...
    def snapshot(self, name, volume, desc=""):
        ...
        self.data_dic = {"desc": desc, "name": name, "volume": volume}
        return self._do_api()
