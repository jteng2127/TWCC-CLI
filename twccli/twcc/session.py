...
class Session2(object):
        ...
        with open(self.twcc_file_session, "w") as fn:
            try:
                documents = yaml.safe_dump(
                    Session2._getSessionData(
                        self.twcc_api_key, self.twcc_proj_code, self.twcc_cid
                    ),
                    fn,
                    encoding="utf-8",
                    allow_unicode=True,
                ...
                raise ValueError(
                    click.style(
                        "Please check Project Code and APIKEY",
                        bg="red",
                        fg="white",
                        blink=True,
                        bold=True,
    ...
    def loadSession(self):
        if self.isValidSession():
            ...
            if isNone(self.sessConf):
                raise ValueError(
                    "{} is not a valid credentials file".format(self.twcc_file_session)
                )
    ...
    def _getDefaultProject(twcc_proj_code=None):
        if isNone(twcc_proj_code):
            if "_TWCC_PROJECT_CODE_" in os.environ and not isNone(
                os.environ["_TWCC_PROJECT_CODE_"]
            ):
                return os.environ["_TWCC_PROJECT_CODE_"]
            if Session2._isValidSession():
                return Session2._isValidSession(isConfig=True)["_default"][
                    "twcc_proj_code"
                ]
    ...
    def __str__(self):
        ...
        key_values = [
            ...
            {"key": "session_created_time", "value": self.sessConf["_meta"]["ctime"]},
            {"key": "twcc_cli_version", "value": self.sessConf["_meta"]["cli_version"]},
            {
                "key": "twcc_apikey_owner",
                "value": Session2._whoami(self.twcc_api_key)["display_name"],
            },
        ...
        # return ""
        return table_layout(
            "parameters", key_values, ["key", "value"], isPrint=False, is_warp=False
        )
