...
@click.option("-p", "--port", "port", type=int, required=True, help="Port number.")
@click.option(
    "-s", "--site-id", "site_id", type=int, required=True, help="ID of the container."
)
@click.option(
    "-open/-close",
...
@pass_environment
def vcs(
    env, site_ids, site_id, port, cidr, protocol, is_ingress, fip, portrange, eip
):  # NOSONAR
