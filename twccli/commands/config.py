...
def whoami(ctx):
    ...
    """
    # ctx.verbose = verbose
    ctx.log("Hi, {}, nice to meet you!".format(Session2._whoami()["display_name"]))
    print(Session2())


@click.command(help="Configure the TWCC CLI.")
@click.option(
    "-pcode", "--project-code", "proj_code", help=" TWCC project code (e.g., GOV108009)"
)
@click.option("--apikey", "apikey", required=True, help="TWCC API Key for CLI.")
@click.option(
    "-ua",
    "--user-agent",
    "user_agent",
