...
if sys.version_info[0] == 3 and sys.version_info[1] >= 5:
    ...
    coloredlogs.install(
        level=config["loggers"]["command"]["level"],
...
@click.option(
    ...
    "--show_and_verbose",
    is_flag=True,
    help="Enables verbose mode and show in console.",
)
@pass_environment
...
def check_if_py2():
    ...
    if sys.version_info[0] < 3:
        ...
        if environ.get("TWCC_SHOW_DEPRECATED") is not None:
            __show_deprecated__ = (
                False if environ.get("TWCC_SHOW_DEPRECATED") == "False" else True
            )
        if __show_deprecated__:
            print(
                bcolors.WARNING
                + "******** Warning from TWCC.ai ********\n"
...
def convert_credential():
    ...
    if hdler.isOldCredential():
        click.echo(
            click.style(
                "Old Credential found! Current credential is version: v%s."
                % (hdler.old_version),
                bg="blue",
                fg="white",
                blink=True,
        ...
        if path.exists(self.old_credential):
            with open(self.old_credential, "r") as stream:
                try:
                    ...
                    env_ver_count = (
                        int(_env_ver_[0]) * 10000
                        + int(_env_ver_[1]) * 100
                        + int(_env_ver_[2]) * 1
                    )
                    cli_ver_count = (
                        int(_cli_ver_[0]) * 10000
                        + int(_cli_ver_[1]) * 100
                        + int(_cli_ver_[2]) * 1
                    )
                    online_ver_count = (
                        int(_online_ver_[0]) * 10000
                        + int(_online_ver_[1]) * 100
                        + int(_online_ver_[2]) * 1
                    )
...
def _fetch_file(url, save_to):
    open(save_to, "wb").write(requests.get(url, allow_redirects=True).content)
