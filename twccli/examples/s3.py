...
@click.command()
@click.option(
    "-n", "--name", "bucket_name", required=True, type=str, help="Name of the Bucket"
)
...
@click.option(
    ...
    "df",
    is_flag=True,
    help="Help delete all the files inside the bucket before delete bucket.",
)
...
@click.option(
    "-d", "--directory", "directory", required=True, help="Name of the Bucket."
)
        ...
        if key.endswith("*"):
            ...
            for desire_file in desire_files:
                if not desire_file.endswith("/"):
                    ...
                    s3.download_bucket(
                        file_name=new_directory, bucket_name=source, key=desire_file
                    )
        else:
