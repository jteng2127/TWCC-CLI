...
@click.command()
@click.option(
    "-cntr", "cntr_name", default="twcc-cli", type=str, help="Enter containr name"
)
@click.option("-gpu", default=1, type=int, help="Enter number of gpu")
@click.option(
    "-sol", "sol_name", default="TensorFlow", type=str, help="Enter solution name"
)
...
@click.option(
    "-table", "isTable", default=True, type=bool, help="Show cntr info in table style."
)
...
def create_commit():
    ...
    site_id = click.prompt("Please Input the site ID which you would like to commit: ")
    tag = click.prompt("Please Input the image tag  ")
    image = click.prompt("Please Input the image name: ")
    c = image_commit()
